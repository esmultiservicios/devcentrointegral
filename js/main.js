...
$(".FormularioAjax").submit(function (e) {
  ...
  // Identifica el botón dentro del formulario modal
  var botonModal = form.find('button[type="submit"]');
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (datos[6] == "AtencionMedica") {
            showFactura(datos[8]); // Función se encuentra en myjava_atencion_medica.js
          }

          if (datos[6] == "Facturacion") {
            pago(datos[8]); // Función para realizar el pago
            pagination(1);
          }
          ...
          if (datos[6] == "FacturacionCredito") {
            printBill(datos[8]); // Función para imprimir la factura
            pagination(1);
          }
  ...
  document
    ...
    .forEach(function (el) {
      let parent = el.closest(
        ".form-group, .form-row, .col-md-4, .col-sm-3, .col-sm-5, .col-sm-2",
      );
      if (!parent) return;
      ...
      // 🔎 Buscar si ya tiene asterisco manual
      let manual =
        label.querySelector(".priority") || label.innerHTML.includes("*");

      if (manual) {
        // ✅ Si ya tiene *, solo asegurar estilo
        let span = label.querySelector(".priority");

        if (!span) {
          // Si el * está como texto, lo envolvemos en span
        ...
        // ✅ Si no tiene *, lo agregamos automático
