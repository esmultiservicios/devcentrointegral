...
function acciones_fact_empresa($fact_empresas_id) {
    $fact_empresas_id = (int)$fact_empresas_id;

    return '
        <div class="dropdown">
            <button class="btn btn-sm btn-primary dropdown-toggle px-3" type="button" id="dropdownFactEmpresa'.$fact_empresas_id.'" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <i class="fas fa-cog"></i> Opciones
            </button>

            <div class="dropdown-menu dropdown-menu-right shadow" aria-labelledby="dropdownFactEmpresa'.$fact_empresas_id.'" style="min-width:220px;">

                <a class="dropdown-item" href="javascript:editarRegistro('.$fact_empresas_id.');void(0);">
                    <i class="fas fa-edit text-primary mr-2"></i> Editar empresa
                </a>

                <div class="dropdown-divider"></div>

                <a class="dropdown-item text-danger" href="javascript:modal_eliminar('.$fact_empresas_id.');void(0);">
...
if ($dato !== "") {
    $where[] = "(
        nombre LIKE ?
        OR rtn LIKE ?
        OR CAST(fact_empresas_id AS CHAR) LIKE ?
    )";
...
try {
    ...
    while ($registro2 = $result->fetch_assoc()) {
        ...
        $fact_empresas_id = (int)$registro2['fact_empresas_id'];
        ...
        $tabla .= '
            <tr>
                ...
                <td>'.$nombre.'</td>
                <td>'.$rtn.'</td>
                <td>'.acciones_fact_empresa($fact_empresas_id).'</td>
            </tr>
        ';
